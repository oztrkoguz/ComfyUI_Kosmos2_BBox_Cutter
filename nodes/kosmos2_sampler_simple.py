        ...
        for im in image:
            ...
            if cut == True:
                ...
                for coord in coordinates: 
                    white_image[int(coord[1]):int(coord[3]),int(coord[0]):int(coord[2])] = image_np[int(coord[1]):int(coord[3]),int(coord[0]):int(coord[2])]
                torch_tensor = torch.from_numpy(white_image)
                torch_tensor = torch_tensor.unsqueeze(0)
                return (descriptions, entity_str,torch_tensor,)
            ...
            if bbox == True:
                ...
                return (descriptions, entity_str,torch_tensor2,)         
        return (descriptions, entity_str,image)



NODE_CLASS_MAPPINGS = {

    "KosmosLoader": KosmosLoader,
    "Kosmos2SamplerSimple" : Kosmos2SamplerSimple

}
# A dictionary that contains the friendly/humanly readable titles for the nodes
NODE_DISPLAY_NAME_MAPPINGS = {

    "KosmosLoader": "Kosmos2 Loader",
    "Kosmos2SamplerSimple" : "Kosmos2 Sampler Simple"
